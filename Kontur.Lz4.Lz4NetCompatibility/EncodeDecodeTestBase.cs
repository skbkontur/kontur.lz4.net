...
{
    public abstract class EncodeDecodeTestBase
    {
            ...
            [Values(true, false)] bool knownOutputSize)
        {
            ...
            using (var file = File.Open(Path.Combine(TestContext.CurrentContext.TestDirectory, fileName), FileMode.Open, FileAccess.Read))
            {
                for (var i = 0; i < 100; i++)
                {
                    ...
                    Decode(encoded, offsetDecode, encoded.Length - offsetDecode, decoded, offsetInDecodedBuffer,
        ...
        protected abstract byte[] Decode(byte[] encoded, int offsetDecode, int i, int lengthForEncodeInput);

        [TestCase(0, 3)]
        [TestCase(3, 3)]
        [TestCase(0, 0)]
        [Repeat(100)]
        public void TestOldEncodeNewDecodeOnRandomData(int offsetEncode, int offsetDecode)
        {
            var rand = new Random();
        ...
        protected abstract byte[] Encode(byte[] original, int offsetEncode, int lengthForEncodeInput);
    }
}